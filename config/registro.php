<?php
    ...
    if (isset($_POST['nombre_completo']) && isset($_POST['no_empleado']) && isset($_POST['usuario']) && isset($_POST['contraseña']) && isset($_POST['c_contraseña']) && isset($_POST['correo_electronico'])) {
        ...
        $no_empleado = validar($_POST['no_empleado']);
        ...
        $datos_usuario = 'nombre_completo=' . $nombre_completo . 'no_empleado=' . $no_empleado . 'usuario=' . $usuario . 'correo_electronico=' . $correo_electronico;
        
        if (empty($nombre_completo)) {
            header("Location: ../pages/registrar_usuario.php?error=Se requiere nombre completo&$datos_usuario");
            exit();
        } elseif (empty($no_empleado)) {
            header("Location: ../pages/registrar_usuario.php?error=Se requiere número de empleado&$datos_usuario");
            exit();
            ...
            if (mysqli_num_rows($query) > 0) {
                ...
                $sql2 = "INSERT INTO usuarios(nombre_completo, no_empleado, usuario, contraseña, correo_electronico) VALUES ('$nombre_completo', '$no_empleado', '$usuario', '$contraseña', '$correo_electronico')";
